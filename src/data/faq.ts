...
export const homeFaq: Faq[] = [
  ...
  { q: 'What if I forget my passphrase?', a: 'Export a Recovery Kit once and store its recovery secret somewhere separate. Kit plus secret lets you set a new passphrase on the same vault. Without the passphrase or the kit and secret together, the vault cannot be opened, by anyone, by design.' },
  { q: 'What is the difference between a backup and a Recovery Kit?', a: 'A normal .aegis backup holds an encrypted snapshot of your data and restores as a new vault identity, so it never reveals your live keys. A Recovery Kit holds only your wrapped vault identity and no passwords. Keep both: the backup for data, the kit and secret for identity.' },
  ...
  { q: 'Has Aegis been independently audited?', a: 'Not by an outside firm yet. Version 2 went through an adversarial design review, browser validation in Chromium and Firefox, a 79.8 million input fuzzing campaign and security gates in CI, and none of that is a claim of formal verification. An independent professional audit is planned before Teams leaves early access, and the report will be linked on the Security page when it exists.' },
];
...
export const personalFaq: Faq[] = [
  ...
  { q: 'I have a vault from version 0.1. What happens?', a: 'It migrates to the v2 format the next time you unlock it, using your existing passphrase, and you are given a new recovery secret to store. Old recovery keys do not open v2 vaults, so export a fresh Recovery Kit after migrating.' },
...
export const businessFaq: Faq[] = [
  { q: 'Can an administrator recover an employee vault?', a: 'Only through an escrow policy the organisation turns on deliberately, where a Recovery Kit and its secret are held for the admin at enrolment. Without that policy, nobody but the user can open the vault.' },
